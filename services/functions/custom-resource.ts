...
import { getOptions } from "@img-omg-img/core/image/image-handler/lib/get-options";
import { isNullOrWhiteSpace } from "@img-omg-img/core/image/image-handler/lib/helper";
import {
  ...
  ErrorCodes,
  CustomResourceError,
} from "@img-omg-img/core/image/custom-resource/lib";
...
export async function handler(
  ...
  try {
    switch (ResourceProperties.CustomAction) {
      case CustomResourceActions.SEND_ANONYMOUS_METRIC: {
        const requestProperties: SendMetricsRequestProperties =
          ResourceProperties as SendMetricsRequestProperties;
        if (requestProperties.AnonymousData === "Yes") {
          response.Data = await sendAnonymousMetric(
            requestProperties,
            RequestType
          );
        }
    ...
    console.error(
      `Error occurred at ${event.RequestType}::${ResourceProperties.CustomAction}`,
      error
    );
...
async function sendAnonymousMetric(
  ...
  try {
    ...
    const payload: MetricPayload = {
      ...
      TimeStamp: dayjs.utc().format("YYYY-MM-DD HH:mm:ss.S"),
      Data: {
        Region: AWS_REGION!,
        Type: requestType,
...
async function putConfigFile(
  ...
  for (let retry = 1; retry <= RETRY_COUNT; retry++) {
    try {
      ...
      if (
        retry === RETRY_COUNT ||
        (error as any)?.code !== ErrorCodes.ACCESS_DENIED
      ) {
        console.info(
          `Error occurred while putting ${DestS3key} into ${DestS3Bucket} bucket.`,
          error
        );
...
async function copyS3Assets(
  ...
  const { ManifestKey, SourceS3Bucket, SourceS3key, DestS3Bucket } =
    requestProperties;
...
async function validateBuckets(
  requestProperties: CheckSourceBucketsRequestProperties
): Promise<{ Message: string }> {
  const { SourceBuckets } = requestProperties;
  const buckets = SourceBuckets.replace(/\s/g, "");

  console.info(
    `Attempting to check if the following buckets exist: ${buckets}`
  );
...
async function checkSecretsManager(
  ...
  if (isNullOrWhiteSpace(SecretsManagerName)) {
    throw new CustomResourceError(
      "SecretNotProvided",
      "You need to provide AWS Secrets Manager secret."
    );
  }
  ...
  if (isNullOrWhiteSpace(SecretsManagerKey)) {
    throw new CustomResourceError(
      ...
      "You need to provide AWS Secrets Manager secret key."
    );
  }
  ...
  for (let retry = 1; retry <= RETRY_COUNT; retry++) {
    try {
      const response = await secretsManager
        .getSecretValue({ SecretId: SecretsManagerName })
        .promise();
      if (response.SecretString && response.ARN) {
        const secretString = JSON.parse(response.SecretString);

        if (
          !Object.prototype.hasOwnProperty.call(secretString, SecretsManagerKey)
        ) {
          throw new CustomResourceError(
            "SecretKeyNotFound",
...
async function checkFallbackImage(
  ...
  const { FallbackImageS3Bucket, FallbackImageS3Key } =
    requestProperties as CheckFallbackImageRequestProperties;

  if (isNullOrWhiteSpace(FallbackImageS3Bucket)) {
    throw new CustomResourceError(
      "S3BucketNotProvided",
      "You need to provide the default fallback image bucket."
    );
  }
  ...
  if (isNullOrWhiteSpace(FallbackImageS3Key)) {
    throw new CustomResourceError(
      "S3KeyNotProvided",
      "You need to provide the default fallback image object key."
    );
  }
  ...
  for (let retry = 1; retry <= RETRY_COUNT; retry++) {
    try {
      data = await s3Client
        .headObject({ Bucket: FallbackImageS3Bucket, Key: FallbackImageS3Key })
        .promise();
      break;
    } catch (error) {
      if (
        retry === RETRY_COUNT ||
        ((error as any)?.code &&
          ![ErrorCodes.ACCESS_DENIED, ErrorCodes.FORBIDDEN].includes(
            (error as any)?.code
          ))
      ) {
        console.error(
...
async function createCloudFrontLoggingBucket(
  requestProperties: CreateLoggingBucketRequestProperties
) {
  const logBucketSuffix = createHash("md5")
    .update(`${requestProperties.BucketSuffix}${dayjs.utc().valueOf()}`)
    .digest("hex");
  const bucketName = `serverless-image-handler-logs-${logBucketSuffix.substring(
    0,
    8
  )}`.toLowerCase();
  ...
  try {
    ...
    console.info(
      `Successfully created bucket '${bucketName}' in '${targetRegion}' region`
    );
  } catch (error) {
  ...
  try {
    ...
    await s3Client
      .putBucketEncryption(putBucketEncryptionRequestParams)
      .promise();
...
async function checkRegionOptInStatus(region: string): Promise<boolean> {
  ...
  const describeRegionsResponse = await ec2Client
    .describeRegions(describeRegionsRequestParams)
    .promise();

  return (describeRegionsResponse?.Regions?.length ?? 0) > 0;
}
