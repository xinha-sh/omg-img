...
import { ImageEdits, ImageFitTypes, ImageFormatType } from "./lib";

export class ThumborMapper {
  private static readonly EMPTY_IMAGE_EDITS = {} as ImageEdits;
  ...
  public mapPathToEdits(path: string): ImageEdits {
    const fileFormat = path.substring(
      path.lastIndexOf(".") + 1
    ) as ImageFormatType;
  ...
  public parseCustomPath(path: string): string {
    ...
    if (path === undefined) {
      ...
      throw new Error(
        "ThumborMapping::ParseCustomPath::RewriteMatchPatternUndefined"
      );
    } else if (REWRITE_SUBSTITUTION === undefined) {
      throw new Error(
        "ThumborMapping::ParseCustomPath::RewriteSubstitutionUndefined"
      );
    } else {
      let parsedPath = "";
      ...
      if (typeof REWRITE_MATCH_PATTERN === "string") {
        ...
        const parsedPatternString = REWRITE_MATCH_PATTERN.slice(
          1,
          REWRITE_MATCH_PATTERN.length - 1 - (flags?.length ?? 0)
        );
  ...
  public mapFilter(
    ...
    fileFormat: ImageFormatType,
    previousEdits: ImageEdits
  ): ImageEdits {
    ...
    switch (filterName) {
      case "autojpg": {
        currentEdits.toFormat = ImageFormatType.JPEG;
        break;
      }
      case "background_color": {
        const colorz = Color("rgb(255, 255, 255)").object();
        const color = !ColorName[filterValue as keyof typeof ColorName]
          ? `#${filterValue}`
          : filterValue;
      ...
      case "blur": {
        const [radius, sigma] = filterValue
          .split(",")
      ...
      case "fill": {
        ...
        if (!ColorName[color as keyof typeof ColorName]) {
          color = `#${color}`;
        }
      ...
      case "format": {
        const imageFormatType = filterValue
          .replace(/[^0-9a-z]/gi, "")
          .replace(/jpg/i, "jpeg") as ImageFormatType;
        const acceptedValues = [
          ImageFormatType.HEIC,
          ImageFormatType.HEIF,
      ...
      case "proportion": {
        ...
        if (currentEdits.resize.width) {
          currentEdits.resize.width = Number(currentEdits.resize.width * ratio);
        }
        if (currentEdits.resize.height) {
          currentEdits.resize.height = Number(
            currentEdits.resize.height * ratio
          );
        }
        break;
      }
      case "quality": {
        const toSupportedImageFormatType = (
          format: ImageFormatType
        ): ImageFormatType | null =>
          [ImageFormatType.JPG, ImageFormatType.JPEG].includes(format)
            ? ImageFormatType.JPEG
            : [
                ImageFormatType.PNG,
                ImageFormatType.WEBP,
        ...
        const targetImageFileFormat = toSupportedImageFormatType(
          currentEdits.toFormat
        );
        // toSupportedImageFormatType(fileFormat)

        if (targetImageFileFormat) {
          currentEdits[targetImageFileFormat] = {
            quality: Number(filterValue),
          };
        }
        break;
      }
      case "rgb": {
        ...
        const values = percentages.map(
          (percentage) => 255 * (Number(percentage) / 100)
        );
        const [r, g, b] = values;
  ...
  private mapCrop(path: string): ImageEdits {
    ...
    if (pathCropMatchResult) {
      const [leftTopPoint, rightBottomPoint] =
        pathCropMatchResult[0].split(":");

      const [leftTopX, leftTopY] = leftTopPoint
        .split("x")
        .map((x) => parseInt(x, 10));
      const [rightBottomX, rightBottomY] = rightBottomPoint
        .split("x")
        .map((x) => parseInt(x, 10));
  ...
  private mapResize(path: string): ImageEdits {
    ...
    if (dimensionsMatchResult) {
      ...
      if (!isNaN(width) && !isNaN(height)) {
        const resizeEdit = { resize: {} } as ImageEdits;
  ...
  private mapFitIn(path: string): ImageEdits {
    return path.includes("fit-in")
      ? ({ resize: { fit: ImageFitTypes.INSIDE } } as ImageEdits)
    ...
    return edits.reduce((result, current) => {
      (Object.keys(current) as Array<keyof typeof current>).forEach((key) => {
        if (Array.isArray(result[key]) && Array.isArray(current[key])) {
          const combinedArr = [...result[key], ...current[key]];
          // @ts-ignore
          result[key] = Array.from(new Set(combinedArr));
