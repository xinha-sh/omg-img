...
export class ImageRequest {
  ...
  public async setup(event: ImageHandlerEvent): Promise<ImageRequestInfo> {
    try {
      ...
      imageRequestInfo.key = this.parseImageKey(
        event,
        imageRequestInfo.requestType
      );
      imageRequestInfo.edits = this.parseImageEdits(
        event,
        imageRequestInfo.requestType
      );
      ...
      imageRequestInfo.headers = this.parseImageHeaders(
        event,
        imageRequestInfo.requestType
      );
      ...
      if (
        ...
        imageRequestInfo.outputFormat = ImageFormatType.PNG;
      }
      ...
      if (
        ...
        if (
          outputFormat &&
          outputFormat === ImageFormatType.WEBP &&
      ...
      if (imageRequestInfo.outputFormat) {
        ...
        if (
          ...
          // TODO: WOW
          const getQualityKeys = Object.keys as <T extends object>(
            obj: T
          ) => Array<keyof T>;
          const qualityKey = getQualityKeys(imageRequestInfo.edits).filter(
            (key) => acceptedValues.includes(key as ImageFormatType)
          )[0];

          if (
            qualityKey &&
            imageRequestInfo.outputFormat &&
            qualityKey !== imageRequestInfo.outputFormat
          ) {
            if (!imageRequestInfo.edits) {
              imageRequestInfo.edits = {} as ImageRequestInfo["edits"];
            } else {
  ...
  public async getOriginalImage(
    ...
    try {
      ...
      const originalImage = await this.s3Client
        .getObject(imageLocation)
        .promise();
      ...
      throw new ImageHandlerError(status, (error as any)?.code, message);
    }
  }
  ...
  public parseImageEdits(
    ...
    } else if (requestType === RequestTypes.THUMBOR && event.path) {
      const thumborMapping = new ThumborMapper();
      return thumborMapping.mapPathToEdits(event.path);
    } else if (requestType === RequestTypes.CUSTOM && event.path) {
      const thumborMapping = new ThumborMapper();
  ...
  public parseImageKey(
    ...
    if (
      ...
      return decodeURIComponent(
        path
          .replace(
            /\/\d+x\d+:\d+x\d+\/|(?<=\/)\d+x\d+\/|filters:[^/]+|\/fit-in(?=\/)|^\/+/g,
            ""
          )
          .replace(/^\/+/, "")
      );
  ...
  public parseRequestType(event: ImageHandlerEvent): RequestTypes {
    ...
    if (path && matchDefault.test(path) && isBase64Encoded) {
      // use sharp
    ...
    } else if (path && matchThumbor.test(path)) {
  ...
  public getOutputFormat(
    ...
    if (
      ...
      const decoded = this.decodeRequest(event);
      return decoded?.outputFormat ?? null;
    }
    ...
    if (ENABLE_SIGNATURE === "Yes") {
      ...
      try {
        ...
        const secret = JSON.parse(
          await this.secretProvider.getSecret?.(SECRETS_MANAGER!)
        );
